...
from src.components.quick_actions import render_quick_actions
...
def render_combatant_card(combatant, index, is_current_turn, view_mode='compact'):
    ...
    if combatant['conditions']:
        status.append(f"⚠️{len(combatant['conditions'])}")
    if combatant['exhaustion'] > 0:
        status.append(f"😫{combatant['exhaustion']}")
    ...
    # Compact format: Name | HP | Init | AC [Status]
...
def render_compact_card(combatant, index, is_current_turn, title, in_dense=False):
    ...
    with st.expander(title, expanded=expanded):
        ...
        with col3:
            st.markdown(f"**Init:** +{combatant.get('initiative', 0)}")
...
def render_detailed_card(combatant, index, is_current_turn, title):
    ...
    with st.expander(title, expanded=is_current_turn):
        ...
        # Notes
        st.markdown("---")
        notes = st.text_area("Notes", value=combatant['notes'], key=f"notes_{index}", height=60)
